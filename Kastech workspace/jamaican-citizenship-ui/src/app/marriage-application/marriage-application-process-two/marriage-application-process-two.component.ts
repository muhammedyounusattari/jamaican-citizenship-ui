...
@Component({
  selector: 'app-marriage-application-process-two',
  templateUrl: './marriage-application-process-two.component.html',
  styleUrls: ['./marriage-application-process-two.component.css']
})
export class MarriageApplicationProcessTwoComponent implements OnInit {

  showErrorMsg: string;
  dob: Date = new Date();
  profile:any;

  marriageApplicationFormProcess2: FormGroup;
  ...
  ngOnInit() {
    this.nonEdittable =  localStorage.getItem('isReviewOnly');
      this.marriageApplicationFormProcess2 = this.formBuilder.group({
      ...
      if(this.nonEdittable){
        this.marriageApplicationFormProcess2.disable();
      }
  }
  ...
  onSave(payload) {
   ...
   next(payload) {
     debugger;
     localStorage.setItem('marriageApplicationFormProcess2', JSON.stringify(payload));
   if(this.nonEdittable)
     this.router.navigate(['/marriageApplication-process3',this.type]);
   else
   ...
   back(payload){
    localStorage.setItem('marriageApplicationFormProcess2', JSON.stringify(payload));
    this.router.navigate(['/marriageApplication']) 
  }
