...
@Component({
  selector: 'app-citizen-registration-process3',
  templateUrl: './citizen-registration-process3.component.html',
  styleUrls: ['./citizen-registration-process3.component.css']
})
export class CitizenRegistrationProcess3Component implements OnInit {

  Register_formthree: FormGroup;
  dob: Date = new Date();
  ...
  ngOnInit() {
    ...
    this.Register_formthree = this.formBuilder.group({ 
    ...
    if(this.nonEdittable){
      this.Register_formthree.disable();
    }
  ...
  back(payload){
    localStorage.setItem('Register_formthree',JSON.stringify(payload));
    if(this.nonEdittable)
  ...
  if(this.nonEdittable)
    this.router.navigate(['/citizenRegistration-process4',this.type]);
  else
    this.router.navigate(['/citizenRegistration-process4']);
  }
