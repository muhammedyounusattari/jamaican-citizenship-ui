...
export class NatrualizationProcessRequest6Component implements OnInit {
  ...
  constructor(private datePipe: DatePipe,private formBuilder:FormBuilder,private router:Router, private activatedRoute:ActivatedRoute, private naturialisationService:NaturlizationService,private utilityService:UtilityService) { 
  ...
  uploadSubmit() {
    this.process6.value.email = JSON.parse(localStorage.getItem('roles')).email;
